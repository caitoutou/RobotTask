...
import XRobot
from XRobot.commons.renderers import MjRenderer
import XRobot.envs
import XRobot.robots
from XRobot.robots.base import BaseRobot

ROBOPAL_PATH = os.path.dirname(inspect.getfile(XRobot))
MJCF_PATH = os.path.join(ROBOPAL_PATH, "assets/XRobot.xml")


def make(env_name: str, **kwargs) -> "MujocoEnv":
    """ Create an environment instance. """
    return XRobot.envs.REGISTERED_ENVS[env_name](**kwargs)


class MujocoEnv:
    """ This environment is the base class.

        :param XRobot(str): Load xml file from xml_path to build the mujoco model
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(robot, str):
            try:
                self.robot: BaseRobot = XRobot.robots.REGISTERED_ROBOTS[robot]()
            except KeyError:
                logging.error(f"Robot {robot} is not registered. Available robots are {XRobot.robots.REGISTERED_ROBOTS.keys()}.")
                raise KeyError
        else:
            self.robot = robot()
        assert isinstance(self.robot, BaseRobot), "Please select a XRobot config file."
